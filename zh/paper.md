...
非牛顿视频流变~\cite{hamamichi2023nonnewtonian} 弥补这一空缺:从一次简单溃坝流估计 Herschel--Bulkley 参数 $(\sigma_Y, \eta, n)$——在一个优化器内部反复执行物质点法 (MPM) 前向模型,直到仿真流再现观测剪影。该方法天然能处理仪器流变仪无法处理的材料,但被它的内循环卡住:一次 MPM 溃坝在现代 GPU 上要数分钟,无导数优化要在收敛前发数百次候选,因此端到端反演\emph{每个 setup 大约八小时}。这一延迟把方法困在离线研究用途,排除了一个更快的视频方法本该解锁的三种能力:
...
我们的工作继承自~\citet{hamamichi2023nonnewtonian} 的问题设定、溃坝观测协议和反演形式——包括他们为应对屈服应力 / 稠度退化而引入的多 setup 联合反演——并贡献了使该形式在计算上切实可行的机器。通过把反演移植到一个学习好的代理之上,同样的 setup 选择与两 setup 联合机器变成每材料可承担,而非只对每个 showcase 才付得起,而反演返回时间在上述三种用途所需的延迟之内。
