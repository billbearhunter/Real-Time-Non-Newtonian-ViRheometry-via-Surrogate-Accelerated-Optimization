...
我们的验证方法是:在真实视频采集上对比模拟在环反演前作,在流变仪可以处理的材料上对比平板流变仪测量,并发现恢复出来的参数与前作持平,而延迟低三个数量级。秒级反演解锁两种在每个 setup 数小时代价下不可达的用途:\emph{(i)}~\emph{时变流体的新鲜参数估计}——反演返回的是材料当前状态而不是数小时前的过时快照;以及 \emph{(ii)}~\emph{真实流体可重复验证}——流变仪锚定的材料 panel 与多 setup 会话能放进一次单用户交互而非离线 showcase。

---

## 1. Introduction

许多日常流体——酸奶、糊、酱料、浆、化妆品乳液——表现出非牛顿行为,这种行为决定了它们如何倾倒、铺展、附着、形变。平板与锥板流变仪通过受控间隙剪切流测量这种行为,但在含有大于测量间隙的内含物的材料,以及流变性质演化速度快于实验流程能够捕捉的材料上失效。

非牛顿视频流变~\cite{hamamichi2023nonnewtonian} 弥补这一空缺:从一次简单溃坝流估计 Herschel--Bulkley 参数 $(\sigma_Y, \eta, n)$——在一个优化器内部反复执行物质点法 (MPM) 前向模型,直到仿真流再现观测剪影。该方法天然能处理仪器流变仪无法处理的材料,但被它的内循环卡住:一次 MPM 溃坝在现代 GPU 上要数分钟,无导数优化要在收敛前发数百次候选,因此端到端反演\emph{每个 setup 大约八小时}。这一延迟把方法困在离线研究用途,排除了一个更快的视频方法本该解锁的两种能力:

- \emph{时变流体的新鲜参数。}当材料的性质在测量窗口内演化——沉降、老化、温度漂移、混合历史、烹饪冷却——一个小时尺度的反演返回的是描述过去某一状态的参数。等到结果给出来,所推断的值已经过期。
- \emph{真实流体的可重复验证。}比较多个流变仪锚定的材料,或者重复 setup,需要把反演运行多次。在每个 setup 数小时的代价下,这类 panel 只能停留在离线 showcase;在每个 setup 数秒的代价下,它们可以放进一次普通的测量会话内。

我们提出一个代理加速框架,把反演从每个 setup 数小时压缩到几秒,使这两种 payoff 在一个实用工作流内可用。我们的技术核心耦合三个组件。
...
我们的工作继承自~\citet{hamamichi2023nonnewtonian} 的问题设定、溃坝观测协议和反演形式——包括他们为应对屈服应力 / 稠度退化而引入的多 setup 联合反演——并贡献了使该形式在计算上切实可行的机器。通过把反演移植到一个学习好的代理之上,同样的 setup 选择与两 setup 联合机器变成每材料可承担,而非只对每个 showcase 才付得起,而反演返回时间在上述两条 payoff 所需的延迟之内。
...
我们的前向仿真器继承了由~\cite{stomakhin2013material,jiang2015affine,hu2018moving,jiang2016material} 带入图形学的 MPM 脉络,屈服应力流变性通过 return-mapping 形式实现~\cite{yue2015continuum,ram2015material,klar2016drucker,tampubolon2017multi,wolper2019cdmpm}。我们在 Taichi~\cite{hu2019taichi} 上采用~\citet{yue2015continuum} 的 Herschel--Bulkley return map;来自前作同一实验室的~\citet{nagasawa2019mixing} 与我们关心的材料类别一致。


### 2.2 Video-Based Material Estimation

通过把渲染动力学与观测动力学匹配来从视频估计仿真参数,在图形学与视觉中有长久的脉络~\cite{bhat2002computing,monszpart2016smash}。\citet{takahashi2019video} 通过逐场景匹配渲染剪影与观测溃坝剪影确立了"视频到参数"的范式(针对粘性流体);\citet{hamamichi2023nonnewtonian} 把这一范式扩展到 Herschel--Bulkley 三元组 $(\sigma_Y,\eta,n)$,是我们工作的直接前作。我们继承他们的溃坝观测协议、反演目标与模拟在环 formulation(包括他们为应对屈服应力 / 稠度退化而引入的多 setup 机器),贡献的是代理本身,使同一套机器对每材料都付得起,而不只是对 showcase 付得起。
...
高斯过程回归~\cite{rasmussen2006gaussian} 把灵活的非参数回归与校准的预测不确定度耦合在一起,是昂贵模拟器的天然代理;稀疏与变分近似~\cite{snelson2006sparse,quinonero2005unifying,titsias2009variational,hensman2013gaussian,matthews2016sparse} 把它扩展到精确 GP 数据规模之外。面对非均质局部结构,专家混合模型~\cite{jordan1994hierarchical,yuksel2012twenty} 把预测通过门控机制分解到多个专家上;GP 专家混合的变体~\cite{tresp2001mixtures,rasmussen2002infinite,meeds2006alternative,nguyen2008local,yuan2008variational,trapp2020deep} 用 GP 实例化这一思路,Bayesian Committee Machine~\cite{tresp2000bayesian,cao2014generalized,deisenroth2015distributed,liu2018generalized} 闭式方差加权融合独立 GP 预测(近期被应用到物理仿真模拟~\cite{cohen2024sparse})。

反演昂贵模拟器在贝叶斯校准~\cite{kennedy2001bayesian} 与基于仿真的推断~\cite{cranmer2020frontier} 两个名下被研究,主流搜索策略是 efficient global~\cite{jones1998efficient} 与 Bayesian~\cite{snoek2012practical,shahriari2016taking} 优化;"在 GP 代理上做无导数优化"~\cite{bajaj2019gaussian,hansen2016cma} 已应用到反向粘塑性~\cite{vandamme2021bayesian} 与粘弹性~\cite{hashemi2025bayesian}。这种替换本身不是启发式:\citet{stuart2018posterior} 用代理预测误差的矩界住代理后验与真实模拟器后验之间的 Hellinger 距离,\citet{teckentrup2020convergence} 把这一保证推广到核超参由数据估计的现实情形。


### 2.5 Camera Calibration

从图像估计相机位姿与内参是计算机视觉的基础任务~\cite{hartley2004multiple}。基于标志物的 fiducial——AprilTag~\cite{olson2011apriltag}、ARTag~\cite{fiala2005artag}、ArUco / ChArUco~\cite{garrido2014automatic,garrido2016generation}——给出 3D-2D 对应关系,可由 PnP 变种(如 EPnP~\cite{lepetit2009epnp})求解,亚像素角点精化~\cite{forstner1987fast} 锐化对应关系。基于剪影或边缘的精化通过 Canny 边缘~\cite{canny1986computational} 的欧氏距离变换~\cite{borgefors1986distance,maurer2003linear} 上的 chamfer-distance 优化把相机对齐到物体边界。我们的管线(\S\ref{sec:calib})把 ChArUco 初始化与两阶段剪影边缘精化结合,替代~\cite{hamamichi2023nonnewtonian} 所用的手动 fSpy + Photoshop 流程。
...
\paragraph{实现细节。}
容器壁使用 sticky 边界(每步把内部网格单元上的动量清零,穿壁粒子从剪影中排除);流体表面是一个带符号距离场,通过 Laplacian-then-biharmonic 平滑提取,每一步把单元 $\varphi$ clamp 到局部粒子半径允许的范围 $[\varphi_{\min},\,\varphi_{\max}]$,移除早期管线中逐帧 $\varphi$ 调整的需要;渲染剪影由零等值面的 marching-cubes 网格化产生,通过~\S\ref{sec:calib} 的标定相机 $(\mathbf{K}_{0}, R^{*}, \mathbf{t}^{*})$ 投影,把从粒子到 $\mathbf{y}$ 的回路闭合。
...
当目标材料本身在测量窗口内演化(沉降、老化、温度漂移、烹饪冷却),真实状态 $\boldsymbol{\theta}_{t}$ 在反演期间漂移,小时尺度的估计就不再描述当前状态;我们因此希望延迟短到足以\emph{跟踪}演化中的状态,而不只是降低摊销后的计算成本。
...
$$
    ...
    \tag{eq:simprecision}
$$
其中 $\{Y_{s,i,\cdot}\}_i$ 是分配给子专家 $s$ 的轨迹,$\alpha_d$ 是反演损失使用的 frame weights(\S\ref{sec:inv-nn};表~\ref{tab:hyper})。每个 profile 都返回单一主子专家 $s_{\mathrm{prod}}\in\{s_{\mathrm{real}},s_{\mathrm{sim}}\}$。两 setup 路径把第一 setup 作为 top-$1$ anchor,把被路由的 setup-$2$ 候选作为离散联合假设;profile 只控制三个旋钮:路由分数、setup-$2$ 候选个数 $M$、以及候选选择规则(\S\ref{sec:mainresults} 使用 \texttt{sim\_precision}、$M=3$ 与首观测 WRMS guard;\S\ref{sec:rheometer} 使用 \texttt{real\_robust}、$M=10$ 与保守选择)。在被观测的 $(W,H,\mathbf{y}_{\mathrm{obs}})$ 而不是未知 $\boldsymbol{\theta}$ 上路由避开了一个循环 gate,并使越界情形通过检查所选子专家的训练 box 而可被诊断。
...
在线前向因此为:通过~(\ref{eq:gate})--(\ref{eq:simprecision}) 把 $(W,H,\mathbf{y}_{\mathrm{obs}})$ 路由出 $s_{\mathrm{prod}}$,按~(\ref{eq:xscale}) 标准化输入,通过~(\ref{eq:gp-predictive}) 求被选 ExactGP 的值,把均值解码到物理单位并应用~(\ref{eq:mono}) 的单调守门。GP 同时返回一个预测标准差,供~\S\ref{sec:inv-loss} 的反演损失消费。
...
\label{sec:inv-sy}
我们不用人工选择的先验打破残余的 $(\eta,\sigma_Y)$ 可识别 ridge,而是通过 CMA-ES 多重启散度与局部 Laplace--Hessian 置信区间(\S\ref{sec:inv-cma})把它暴露出来,使报告的不确定度被绑定在似然上,而非绑定在用户选定的锚定上。
...
\label{sec:inv-cma}
我们用多重启 CMA-ES~\cite{hansen2016cma} 最小化~(\ref{eq:Pprime}),其自适应采样分布很适合~(\ref{eq:obs-loss}) 产生的分段光滑、ridge 易出现的曲面。每次重启用确定性种子,最低损失重启被报告为 MAP,重启之间的标准差是一个 truth-blind 的 ridge 诊断,在 MAP 处做有限差分 Hessian 给出 Laplace 95\% 区间。每一代 CMA-ES 由一次批量 GP 前向调用估值((\ref{eq:gp-predictive}) 应用于堆叠的 $\mathbf{X}_*$)~\cite{rasmussen2006gaussian},把 GPU 派发开销摊销到 $\lambda$ 个候选之上,且不改变优化轨迹。种群大小、重启次数与其它超参数列在~\S\ref{sec:data}。

\paragraph{联合两 setup 反演。}
\label{sec:inv-joint}
对同一材料的两次观测,被路由的两个 setup 各自保留自己的 GP 专家,但共享同一个 $\boldsymbol{\theta}$。生产实现在两个填充子专家 box 的交集(或在不相交时填充并集)上最小化~(\ref{eq:Pstar}),从单 setup 最优解温启动,使第二次观测在第一后验之\emph{上}贡献信息,而不是重启搜索。

\begin{algorithm}[!htbp]
\caption{在线反演(单 setup;当提供第二 setup $(\mathbf{y}^{(b)},W_b,H_b)$ 时为联合反演)。}
\label{alg:invert}
\KwIn{$(\mathbf{y}_{\mathrm{obs}},W,H)$,以及可选的 $(\mathbf{y}^{(b)},W_b,H_b)$;生产 GP bank。}
\KwOut{$\hat{\boldsymbol{\theta}}$ 与后验诊断。}
\BlankLine
按部署 profile((\ref{eq:y8q})、(\ref{eq:simprecision}))把每个提供的 setup 路由到其主子专家\;
构造填充搜索 box(式~(\ref{eq:tight-box});两 setup 时为两个逐子专家 box 的交集,不相交时用填充并集)与第一 setup 上的最近邻温启动(式~(\ref{eq:nn}))\;
对~(\ref{eq:Pprime})(单 setup)或~(\ref{eq:Pstar})(联合,从单 setup 最优解温启动)跑多重启 CMA-ES\;
可选:在最优点求局部 Laplace Hessian 区间\;
...
$(W_2, H_2)$ 的选择不属于我们的贡献:我们原样采用~\citet{hamamichi2023nonnewtonian} 的闭式、基于 Fisher 信息的提议器。给定单 setup 估计 $\hat{\boldsymbol{\theta}}_1$,他们用平面 Poiseuille 流近似溃坝,推导出解析损失 Hessian $\mathbf{H}(\hat{\boldsymbol{\theta}}_1; W,H)$。设 $\mathbf{q}_1$ 是 $\mathbf{H}(\hat{\boldsymbol{\theta}}_1; W_1, H_1)$ 最小特征值对应的单位特征向量——$\boldsymbol{\theta}$ 空间里第一个 setup 携带最少信息的方向。提议器选取
$$
    (W_2^\star, H_2^\star)
    \;=\;
    ...
    \tag{eq:proposer}
$$
使联合 Fisher 信息在 $(\mathbf{q}_1, \mathbf{q}_2)$ 子空间里满秩。Hessian 是闭式的,因此求~(\ref{eq:proposer}) 不需要任何前向仿真。我们的贡献是把消耗该提议器输出的**两 setup** 联合估计变成每材料而不是每 showcase 可承担;joint 信息增益依赖每种材料的残余 ridge 几何,在~\S\ref{sec:rheometer} 的流变仪锚定真实材料上表现最明显(稠度参数、ridge 收紧),而在~\S\ref{sec:mainresults} 的合成 $N=30$ Hamamichi-对齐面板上,我们的 replay-consistency guard 让 joint update 保留(而非倒退)单 setup 的恢复(表~\ref{tab:setup-ablation})。前作另外报告的级联 $3$/$4$-setup 改进——每一步只对前一步未通过的 case 加跑——是同一 proposer 与 surrogate 的自然延伸,但本文未实现。
...
$$
    ...
    \tag{eq:huber}
$$
它阻止任何单个未精化好的角点主导拟合。~(\ref{eq:stage1}) 用 Nelder--Mead~\cite{nelder1965simplex} 最小化,返回 $\mathbf{K}_{0}$(由 $\mathrm{fov}$ 与图像尺寸推出)和初始外参 $(R_{0}, \mathbf{t}_{0})$。

\paragraph{Stage 2:剪影引导的外参精化。}
Stage~1 通常精确到几个像素,但 ChArUco 板与容器并不刚性耦合;Stage~2 固定 $\mathbf{K}_{0}$ 而仅精化外参,使渲染的容器剪影对齐到溃坝释放瞬间的参考帧 $I_{0}$,分两 phase 进行。

\emph{Phase~A(光滑 chamfer)。}
$I_{0}$ 的 Canny 边集~\cite{canny1986computational} 给出目标边像素集 $E$,我们由此预先计算一个精确欧氏距离变换~\cite{maurer2003linear,borgefors1986distance}
$$
    D_{E}(p) \;=\;
    \min_{q \in E}\,\|\,p - q\,\|_{2}
...
\paragraph{安全 clamp。}
若 $\|\mathbf{r}^{*} - \mathbf{r}_{0}\|$ 或 $\|\mathbf{t}^{*} - \mathbf{t}_{0}\|$ 超过安全阈值($3^{\circ}$ 或 $3$\,cm;比 Phase-A box 略松,使无界的 Phase-B 原则上可越过 box 但只接受温和的越界),回退到 ChArUco 外参,以在 ChArUco 板放置粗糙时防护强局部极小值。
...
$$
    ...
    \tag{eq:xi}
$$
最后,残余的二值化噪声偶尔会在帧之间引入向后跳跃;我们用一次单调 pass $x_{i} \leftarrow \max(x_{i}, x_{i-1})$(等价于在物理空间应用~(\ref{eq:mono}))来防护,产生算法~\ref{alg:invert} 所消费的观测 $\mathbf{y}_{\mathrm{obs}} = (x_{1}, \dots, x_{8}) \in \mathbb{R}_{\ge 0}^{8}$。
...
对 $\boldsymbol{\theta}_{\mathrm{cam}}$ 跑 Nelder--Mead 最小化~(\ref{eq:stage1})、(\ref{eq:huber}) 的 Huber 重投影损失 $\mathcal{L}_{1}$,通过~(\ref{eq:eye})--(\ref{eq:project}) 求 $\pi$\;
...
模拟在环前作的主合成验证报告在 $30$ 个案例上完成($6$ 种材料 $\times\,5$ 个随机初始 setup),单 setup 通过 $11/30$($36.7\%$)、双 setup $20/30$($66.7\%$);随后通过**级联**精化——setup~$k+1$ 只在 setup~$k$ 未通过的 case 上加跑——累计通过率提升到 $3$ setup $26/30$、$4$ setup $30/30$~\cite{hamamichi2023nonnewtonian}。我们把同样的 $30$-case 尺度作为主比较,把更大的面板保留为 robustness check。我们的 $30$ 个案例是从留出 MPM 测试集分层抽样而非前作的特定 $6$ 材料列表;采样协议不同,但面板规模与 $E_{\mathrm{rel}}\le 0.1$ 阈值一致,因此表~\ref{tab:setup-ablation} 中的数字回答的是"反演能否在前作面板规模下正确恢复",而**不是**"我们是否逐案例胜过前作"。在 Hamamichi 的 $6$ 种流变仪拟合参数集上的严格 like-for-like 重跑,以及前作用于清扫剩余失败 case 的级联 $3$/$4$-setup 协议,留作未来工作。

对每个案例,我们先跑单 setup 反演,用~\S\ref{sec:proposer} 的 Hessian-正交提议器选择第二几何,在 ground-truth 参数处用 MLS-MPM 合成该第二观测,然后跑 conservative two-setup joint inverse。two-setup 阶段保留 simulator-matched top-$3$ 个 setup-2 候选(\texttt{sim\_precision} profile 下按 frame-weighted nearest-training-trajectory 距离排序),在每个 candidate pair 自己的 tight sub-expert box 内优化,并且只有当 joint update 不违反第一观测的 truth-blind replay consistency(\S\ref{sec:inv-joint})时才接受。在 $1$ setup 上我们的流水线已经把通过率从前作的 $11/30$ 提升到 $15/30$;在 $2$ setup 上我们停在 $15/30$ 而非滑到前作的 $20/30$,因为我们的 truth-blind guard 拒绝任何会使 setup~$1$ replay 退化的 joint update——前作的无 guard 多 setup CMA-ES 则接受这类 update,因此多转化了 $9$ 个 case。这是一个被显式记录的取舍,不是哪一边的免费胜利:我们的 guard 用面板级转化率换取"绝不覆盖已恢复的单 setup 估计",这是反演被当作每批次仪器(而非每 showcase 研究)使用时必需的行为。完整 $30$-case 面板在一张消费级 GPU 上用 $13.1$\,min 完成;按前作报告的每次单 setup 反演 $\sim\!8$\,h~\cite{hamamichi2023nonnewtonian} 外推,同样 $30$-case 面板模拟在环大致需要 $240$\,h 量级,即约 $10^{3}\times$ 墙上时钟收缩。图~\ref{fig:flow-error-cdf} 给出该面板上参数误差与 forward replay 误差的累积分布。

\begin{table}[!htbp]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{与 Hamamichi 对齐的 $N=30$ 面板上的合成恢复。``$\le 0.1$'' 是按~\eqref{eq:erel} 达到 $E_{\mathrm{rel}}\le 0.1$ 的占比。三列 $\mathrm{relE}$ 是逐参数中位,定义为 $|\hat\theta-\theta^{\!\star}|/\theta^{\!\star}$。斜体行是~\citet{hamamichi2023nonnewtonian} §7.1 在他们自己的尺寸相同但案例独立的 $30$-case 面板上的数字。**前作的 $3$/$4$-setup 行是级联式**:setup~$k+1$ 只在 setup~$k$ 未通过的 case 上加跑,因此 $26/30$ 与 $30/30$ 是增量转化($+6$、$+4$)的累加,而**不是**对所有 $30$ 案例独立跑 $3$ 或 $4$ 次 setup 的结果。我们的流水线目前实现 $1$ 与 $2$ setup;对应的级联 $3$/$4$-setup 行留作未来工作。在 $1$ setup 上我们已经把通过率从前作的 $11/30$ 提升到 $15/30$;在 $2$ setup 上我们停在 $15/30$ 而非前作的 $20/30$,因为我们的 two-setup joint 跑一个 truth-blind first-observation WRMS guard,会拒绝使 setup~$1$ 退化的 joint update(\S\ref{sec:inv-joint})。这是一个被显式记录的取舍:guard 用面板级转化率换取"绝不覆盖已恢复的单 setup 估计",这是反演被当作每批次仪器(而非每 showcase 研究)使用时必需的行为。即使在 guard 下,joint update 仍然把稠度参数收紧($\mathrm{relE}\,\eta$ 从 $0.187$ 降到 $0.160$,相对改善 $14\%$),而 $n$、$\sigma_Y$ 在采样噪声范围内保持不变。完整面板,包括第二 setup 的 MLS-MPM observation synthesis,耗时 $13.1$\,min,而由前作每次反演的墙上时钟外推同一面板大约需要 $\sim\!240$\,h~\cite{hamamichi2023nonnewtonian}。}
...
\begin{figure*}[!htbp]
    ...
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \figureorplaceholder[3.6cm]{\linewidth}{figs/real_world/y8q_milk_single_vs_joint.png}{(c) 保湿乳:流变仪 + 单 setup + 联合曲线;待采集。}\\
        {\footnotesize (c) 保湿乳}
    \end{minipage}\\[6pt]
    % Row 2
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \figureorplaceholder[3.6cm]{\linewidth}{figs/real_world/y8q_lotion_single_vs_joint.png}{(d) 化妆水:流变仪 + 单 setup + 联合曲线;待采集。}\\
        {\footnotesize (d) 化妆水}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \figureorplaceholder[3.6cm]{\linewidth}{figs/real_world/y8q_tonkatsu_single_vs_joint.png}{(e) 猪排酱:流变仪 + 单 setup + 联合曲线;待采集。}\\
        {\footnotesize (e) 猪排酱}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \figureorplaceholder[3.6cm]{\linewidth}{figs/real_world/y8q_sweetbean_single_vs_joint.png}{(f) 甜面酱:流变仪 + 单 setup + 联合曲线;待采集。}\\
        {\footnotesize (f) 甜面酱}
    \end{minipage}
    \caption{Hamamichi 流变仪参考面板上恢复的 HB 流曲线,布局遵循~\citet{hamamichi2023nonnewtonian} 的 Fig.~$13$。点状黑:同批次平板流变仪扫描。红:单 setup 恢复。蓝:联合两 setup 恢复。前作六种参考材料全部报告。Chuno、Okonomiyaki、化妆水(Lotion)、猪排酱(Tonkatsu)上联合曲线在溃坝 $\dot\gamma$ 窗口内与流变仪参考紧贴;Chuno 上单 setup 曲线偏向更高 $\sigma_Y$(预测中的 ridge 位移),联合更新把这一差距闭合。甜面酱(Sweet bean paste)展示同一机制反向作用:单 setup 把 $\sigma_Y$ 估高到 $38$\,Pa,joint 把它拉到 $25$\,Pa 与流变仪参考的 $23$\,Pa 之差 $10\%$ 内,而 $n$ 与 $\eta$ 沿 ridge 互补,使可见的 log--log 流曲线几乎不动 —— 这是\S\ref{sec:inv-sy} 的 $\sigma_Y$/$\eta$ ridge 在参数层(而非曲线层)的清晰真实数据样例。保湿乳(Moisturising milk)是\S\ref{sec:mainresults} 讨论的合成模式 joint 退步的失败模式样例:在 $(W_2=4.5, H_2=2.0)$ 处 setup-2 sub 选到的高-$\dot\gamma$ 证据把恢复的 $n$ 拉向 Newtonian 极限,虽然单 setup 已经接近,joint 曲线在高 $\dot\gamma$ 处反而偏离了流变仪。}
