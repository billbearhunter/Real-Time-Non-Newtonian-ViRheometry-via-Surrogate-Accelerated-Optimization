...
We validate our method against the predecessor's simulation-in-the-loop
benchmark and against parallel-plate rheometer measurements on real
materials the rheometers can handle. The surrogate inverse preserves
dam-break replay and flow-curve agreement in the regimes supported by
the observation, while running at three orders of magnitude lower
latency. The seconds-scale
...
\emph{(ii)}~\emph{practical real-fluid validation and
characterisation}, in which rheometer-referenced anchors,
rheometer-infeasible material panels, and multi-setup sessions fit
inside a single user interaction rather than an offline showcase.
\end{abstract}
