...
unit convention.  The material-only inverse coordinate is
\[
...
The offline partition is fitted once from the clean training
trajectories before inverse experiments.  Validation and test
trajectories may be assigned to the frozen regions for BAE estimation and
forward assessment, but they do not determine GP scalers or
hyperparameters.  For region $s$, let $\mathcal{D}_s$ be its assigned
...
$10/71/4{,}191$ rows per expert. For numerical cost, GP fitting uses at
most $800$ training rows per expert. Each expert then fits
...
vector $\boldsymbol{\rho}_{z,s^\star}$ and clipped to the global HB
bounds,
\begin{equation}
\label{eq:tight-box}
    \mathcal{Z}_{s^\star}
    =
    \mathcal{Z}\cap
    [\mathbf{z}_{\min,s^\star}-p\boldsymbol{\rho}_{z,s^\star},\,
     \mathbf{z}_{\max,s^\star}+p\boldsymbol{\rho}_{z,s^\star}].
\end{equation}
