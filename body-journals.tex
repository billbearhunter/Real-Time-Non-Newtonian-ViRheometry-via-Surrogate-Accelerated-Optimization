...
validation program. We propose a surrogate-accelerated framework that
compresses the inverse from hours to seconds per experimental setup: in the evaluated
...
Multi-setup disambiguation and setup selection remain inherited acquisition protocols,
and they are framed here rather than as a new physical disambiguation
principle.
...
same GP/BAE objective, while the archive lets us report which basins
were covered, retained, or falsely ranked. The validation below shows
that candidate coverage is easier than final ranking.
...
therefore reduce real--simulation-gap sensitivity during expert routing
and candidate anchors only; the final retained-set score uses the unweighted
whitened residual energy described below. This is a deliberate
robustness choice, not a hidden prior on $\boldsymbol{\theta}$: all
retained-set decisions still expose the residual, uncertainty, and
support components separately. The routing schedule is listed in
...
likelihood and max-frame implausibility as diagnostics. This
decomposition is
...
Tab.~\ref{tab:hessian-seq30} reports each accumulated prefix. The
...
The setup pairs in this audit are the measured real-material validation
setups used by the inverse, not pairs selected to maximise
$\dot\gamma$ coverage. This check therefore bounds the shear-rate
regime of the reported real validation; a separate candidate-setup
sweep would be required to claim maximal shear-rate coverage.
...
the same twelve material/setup cases.

The numerical $\hat{\boldsymbol{\theta}}$ values in
Tab.~\ref{tab:rheo-recovery} are reporting representatives, not a claim
that every real material has a unique recovered parameter triple. When
the retained set is single-basin or compact in $z$-space, the reported
value is the best plausible candidate under the GP/BAE ranking rule of
\S\ref{sec:inv-ranking}. When the retained set remains extended along a
ridge, the same rule selects one representative for plotting and replay,
while the ridge width is treated as uncertainty rather than collapsed by
a hand-chosen prior.
...
parameters, pooled over all particles, frames, and the measured
real-validation setup pair for each material. All p5--p95 supports lie
within the measured rheometer interval
$0.1$--$1000\,\mathrm{s}^{-1}$. This is a diagnostic of the validation
regime, not a claim of maximal shear-rate coverage over all possible
setups.}
...
\begin{figure*}[!htbp]
    ...
    curves are regenerated from the reported GP retained-set
...
\begin{figure*}[!htbp]
    ...
    predecessor layout, regenerated with the reported GP inverse result.
...
diagnostic of the reported GP estimates: the mean of per-case mean
...
reported replay behaviour and the
...
We test this consequence as a held-out setup diagnostic. For each
short-time material group, three setups are captured in one session;
two setups are used to estimate a parameter representative, and the
unused third setup is held out for flow-front and binary-mask replay.
...
The evaluated freshness set contains seven held-out flow-front cases
from six material groups. One Oats held-out pair contains flow-front predictions
but no \texttt{snapdiff\_pred} binary-mask files, so it is included in
the flow-front rows and excluded from the mask rows. The largest mask
discrepancy occurs on EggFoam at late frames
($\mathrm{IoU}=0.705$ at frame~8), which is therefore the case to
inspect before interpreting the setup-level diagnostic as evidence for
time-evolving material behaviour.
...
row; operator-dependent capture timing is not used as a headline
result.
...
coverage is strong. However, the best-scored candidate is near truth in
only $7.8\%$, $6.3\%$, $10.9\%$, and $14.1\%$ of the one-, two-,
three-, and four-setup cases. The causal $30$-case audit shows the same
...
rheometer-reference materials, the reported representatives give
$E_{\mathrm{flow}}$ values from $0.200$ to $0.740$ and binary replay
statistics with mean per-case silhouette IoU $0.886$ over twelve
material/setup cases, but these numbers include camera, segmentation,
and real--sim discrepancy. The gamma-dot audit shows that the measured
validation setups excite p5--p95 shear-rate supports inside the
$0.1$--$1000\,\mathrm{s}^{-1}$ rheometer interval, so the flow-curve
comparison is not extrapolated far outside the measured shear-rate
range; it does not prove maximal setup coverage. The freshness panel
shows that a two-setup estimate can replay a third same-session setup
with median $x_{08}$ error $0.203$\,cm, median front error
$0.293$\,cm, mean mask IoU $0.912$, and image XOR fraction $0.0051$.
This supports a repeated-capture protocol enabled by speed, not a
...
The most important limitation is selection, not forward fidelity. The
GP bank is accurate on held-out MPM data, and the retained sets usually
contain a near-truth candidate in synthetic tests, but the lowest-score
candidate is still unreliable. This means that the present method
should be used as an ambiguity-aware inverse: it can report plausible
basins and test whether additional setups shrink them, but it should
not be interpreted as a fully automatic unique-parameter estimator.
Future work should therefore focus on selector validation under
held-out MPM replay, less aggressive plausibility gates, and scoring
rules that are trained or calibrated specifically to rank near-truth
candidates ahead of surrogate-favoured false basins.

The second limitation is coverage. The current surrogate is validated
inside the cleaned fixed-density training box,
$W,H\in[2,7]$\,cm and the sampled HB parameter ranges. The synthetic
tail audit shows weaker retention at the extremes of $\eta$ and
$\sigma_Y$, so extrapolating beyond the sampled box or relying on sparse
tail leaves would be unjustified. A natural extension is targeted MPM
sampling in the low-/high-$\eta$ and high-$\sigma_Y$ regions where the
retained-set test fails most often, followed by the same forward,
BAE, and inverse audits rather than only a new aggregate $R^2$.

The third limitation is physical model discrepancy. The real-material
panel compares against rheometer-reference flow curves and binary-mask
replays, but those observations include camera calibration,
segmentation, wall/contact effects, and real--sim mismatch. The current
MPM model represents HB rheology, gravity, and boundary contact, but it
does not model surface tension, thixotropy, elastic overshoot,
viscoelastic memory, evaporation, or time-dependent ageing. The
freshness experiment should therefore be read as a same-session
held-out setup diagnostic enabled by seconds-scale inverse, not as an
identified kinetic material model. Extending the method to ageing or
thixotropic materials would require time-indexed captures, a chosen
constitutive evolution model, and a separate identifiability analysis.

Finally, the setup and shear-rate analyses are diagnostic rather than
globally optimal. The measured real-validation setup pairs excite
shear-rate supports within the rheometer interval, but we did not sweep
all feasible $(W,H)$ pairs to maximise shear-rate coverage or guarantee
the most informative next setup. A future setup-policy study should
combine the predecessor's Hessian/ridge logic with surrogate
uncertainty and an explicit gamma-dot coverage objective, then evaluate
it by retained-set shrinkage and near-truth survival. The implementation
is also seconds-scale rather than millisecond-scale; this is sufficient
for interactive repeated measurements, but a production system would
benefit from batched GP evaluation, compiled candidate scoring, and
GPU-resident retrieval.
