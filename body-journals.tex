...
A rheometer flow curve and a dam-break video interrogate a material in
different ways. The rheometer imposes controlled, approximately steady
...
sweep; the relevant question is which shear-rate regimes are actually
excited by the real validation setup
...
shows the image-replay binary-mask check in the predecessor-style
snap-difference layout: real foregrounds, MPM foregrounds, and
amplified pairwise differences are shown across the observed frames.
The flow-curve and snap-difference panels may use different diagnostic
candidates because they answer different validation questions. This
separation is intentional: it exposes the real/simulator gap instead of
forcing one point estimate to optimise both the rheometer and image-space
diagnostics. The accompanying contracted silhouette replay packet evaluates all twelve
material/setup cases against the registered real foregrounds; its
mean case-level IoU is $0.886$, with minimum frame-level IoU $0.747$.
...
\begin{figure*}[!htbp]
    ...
    range, and dashed vertical lines mark the HB-fit interval used for
    flow-curve comparison. These plots audit the shear-rate support of
    the real videos; they are not histograms of the inverse candidate
    parameters.}
...
\begin{figure*}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{figs/real_world/real6_current_selected_snapdiff_panel_black.png}
    \caption{Binary snap-difference replay check in the
    predecessor-style layout, regenerated with the replay-diagnostic
    GP candidate. Each row is one real
    material/setup case at frame~8. Columns show the segmented real
    foreground, the GP replay foreground, and the XOR foreground
    difference as white pixels on a black background. This figure is a
    visual diagnostic for sim--real discrepancy under the image-space
    replay candidate; it is not a material-theta selector in
    Tab.~\ref{tab:rheo-recovery}.}
    \Description{Twelve-row binary-mask replay panel for the six real
    materials and two setups per material, showing observed masks,
    diagnostic GP replay masks, and black-background
    white-difference rows at frame 8.}
...
diagnostic of the GP estimates after inference. The diagnostic
flow-curve branch gives mean/worst
