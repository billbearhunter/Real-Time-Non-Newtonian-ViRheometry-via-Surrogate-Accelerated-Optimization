...
forward simulator integrates the modified HB return map of
\S\ref{sec:hb-rheology} on a
moving-least-squares MPM~\cite{hu2018moving} grid with spacing
$\Delta x = 0.126$\,cm and timestep $\Delta t = 7.5\times
10^{-5}$\,s in our replay implementation, implemented on
Taichi~\cite{hu2019taichi}. The
...
Offline, MPM simulations from the quality-controlled corpus at fixed
density $\rho=1.0$ are partitioned into local trajectory regions, and
one exact GP bank is trained for each resulting sub-region. Online, the
observable tuple $(W,H,\mathbf{y}_{\mathrm{obs}})$ defines the inverse
query; the surrogate objective combines trajectory residuals, GP
posterior variance, a local surrogate-residual variance term
(Bayesian approximation error, BAE~\cite{kaipio2005statistical}), and
observation noise. The inverse may search this score with a
derivative-free optimiser, but the reported validation evaluates an
explicit candidate archive so that basin coverage, support, and
retention can be inspected directly. The output is either a supported
point estimate or a retained plausible set. A first setup can also
propose the next physical
...
\begin{taggedeqn}{P$'$}{Pprime}
    ...
    \mathcal{L}_{\mathrm{GP+BAE}}(\mathbf{z};W,H,\mathbf{y}_{\mathrm{obs}})
    \le
...
\begin{taggedeqn}{P$^{\star}$}{Pstar}
    ...
    =
    \sum_{j=1}^{J}
    \mathcal{L}_{\mathrm{GP+BAE}}^{(j)}(\mathbf{z})
    +
    \lambda_{\mathrm{sup}}
...
Scale and model uncertainty
  & front-position residual score using GP variance, BAE variance, and
    observation noise
  & \S\ref{sec:inv-loss} \\
Ridge diagnosis
...
sharing the same expert-local scaled $5$-D input. The implementation
uses a linear mean and a Mat\'ern-$5/2$ automatic-relevance-determination
...
anchor retrieval. They do not alter the final inverse loss: the
validation run reported in \S\ref{sec:mainresults} ranks candidates by
unweighted whitened residual energy under the GP/BAE variance. The
routing schedule is listed in
...
\end{equation}
This term is a ranking and diagnostic signal. It prevents a low
trajectory residual in an extrapolative region from being mistaken for
a well-supported material estimate.
...
and observed front positions in physical trajectory units and combines
the GP epistemic variance, the local BAE residual variance, and an
observation-noise floor. The per-frame variance is
\begin{equation}
\label{eq:gp-var}
    v_d(\mathbf{z})
    =
    ...
    + \sigma_{\mathrm{obs}}^2 .
\end{equation}
The final validation run ranks candidates by the unweighted whitened
residual energy
\begin{equation}
\label{eq:obs-loss}
\begin{aligned}
    ...
    &=
    \sum_{d=1}^{8}
      \frac{r_d(\mathbf{z})^2}{v_d(\mathbf{z})},\\
    r_d&=y_{\mathrm{obs},d}-\hat{y}_d.
\end{aligned}
\end{equation}
The implementation also records the Gaussian negative log likelihood
and max-frame implausibility as diagnostics, but the promoted
$64$-case validation uses the residual-energy score and calibrated
implausibility gates. The variance denominator prevents a candidate
from winning merely because it matches the trajectory under an
unexplained low-variance assumption. For ranking and retention we keep
the score decomposed into explicit diagnostics: trajectory residual, GP
variance, BAE variance, observation noise, and support penalty. This
decomposition is important because the
...
$J_i=\mathcal{L}_{\mathrm{GP}}(\mathbf{z}_i)$,
...
by the same GP/BAE score that a CMA-ES search could optimise. Thus the
method should not be read as a claim that CMA-ES is inferior; the core
change is that the expensive forward call has been amortised and the
inverse decision is reported through explicit residual, uncertainty,
support, and retained-set components.
...
Let $\mathbf{q}_1$ be the unit eigenvector associated with the
smallest eigenvalue of the first setup's Hessian---the direction in
$\boldsymbol{\theta}$-space along which the current setup carries the
least information. The Hessian-orthogonal proposer selects
\begin{equation}
\label{eq:proposer}
    (W_2^\star,H_2^\star)
    \;=\;
...
$t_{1}, \dots, t_{8}$
is binarised by hue-saturation-value (HSV) intensity thresholding
(threshold $128$, black
...
\section{Training Data and Fixed Configuration}
\label{sec:data}

All reported results use a single surrogate bank trained once and
re-used across the inverse experiments. The bank is
trained on the cleaned MPM corpus with fixed density $\rho=1.0$ after removing
non-monotone trajectory rows. The split contains $407{,}480$ clean
rows in total: $325{,}984$ training rows, $40{,}748$ validation rows,
and $40{,}748$ test rows. This partition produces $2{,}775$
local exact-GP experts. The median expert has $71$ assigned rows, the
smallest retained expert has $10$, and the largest has $4{,}191$.
The partition was selected to keep the number of experts in the
$2500$--$2800$ range while keeping the maximum within-leaf $x_{08}$
quantile span below $3$ cm. The global parameter box spans
...
\caption{Data-integrity gate for the cleaned MPM corpus with fixed density $\rho=1.0$. The
clean split is the only source used to train and validate this
GP bank; excluded rows are retained only as cleaning diagnostics.}
...
The split audit additionally found no duplicate
$(n,\eta,\sigma_Y,W,H)$ keys and no non-monotone rows inside the
train/validation/test CSVs. One practical consequence is that the
clean CSV split is a single-observation forward corpus, not a
same-material multi-setup replay dataset. Synthetic multi-setup
inverse validation therefore constructs additional observations by
replaying or generating the same $\boldsymbol{\theta}$ under new
setups, rather than assuming repeated same-theta setup groups already
exist in the split.

\begin{table}[!t]
\begingroup
\centering
\caption{Fixed configuration used throughout the evaluation.}
\label{tab:hyper}
\scriptsize
...
\quad expert row counts             & min / median / max =
                                      $10$ / $71$ / $4{,}191$ \\
