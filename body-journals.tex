...
declares $E_{\mathrm{rel}}\le0.1$ as good enough, and uses additional
setups to disambiguate failures. We keep the same two principles:
synthetic observations must have known ground truth, and later setups
must be interpreted as disambiguation evidence rather than as a
separate physical model. Replacing MPM by a GP surrogate adds a new
...
accuracy, retained-set size, and setup separability rather than
...
\paragraph{Setup-policy separation diagnostic.}
The main pipeline uses the inherited Hessian-orthogonal setup policy,
but the current GP evidence supports it only as a separability
diagnostic.  We therefore evaluate setup proposals by asking whether a
candidate setup would make retained representatives produce more
distinct predicted trajectories.  Tab.~\ref{tab:setup-separation}
reports the median tenth-percentile standardized pairwise separation
over eight retained-representative cases; larger values indicate that
even close pairs of representatives are easier to tell apart under the
proposed setup.  The diagnostic confirms that setup choice changes
candidate separability, but it does not establish a new recovery claim:
the retained-set results in Tab.~\ref{tab:retained-contract} remain the
main synthetic inverse evidence.

\begin{table}[!htbp]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{Setup-policy separation diagnostic on eight retained-set
cases.  Entries are medians of the tenth-percentile standardized
pairwise trajectory separation; higher values mean stronger predicted
disambiguation among retained representatives.}
\label{tab:setup-separation}
\begin{tabular}{@{}lcccc@{}}
\toprule
Policy & setup 1 & setup 2 & setup 3 & setup 4 \\
\midrule
Fixed setup sequence & $0.134$ & $0.226$ & $0.202$ & $0.251$ \\
Hessian-orthogonal & $0.144$ & $0.183$ & $0.205$ & $0.162$ \\
GP-OED separation diagnostic & $0.169$ & $0.235$ & $0.218$ & $0.208$ \\
\bottomrule
\end{tabular}
\end{table}
...
errors can hide failures at the small- and large-value tails.  The
current contract reports retained-set coverage over the full held-out
panel; it is not used to make a separate tail-specific recovery claim.
...
mean silhouette IoU is $0.886$, the worst per-frame IoU is $0.747$,
and the mean foreground-error fraction is $0.0071$.
...
silhouette IoU is $0.886$ across the twelve material/setup cases, with
worst per-frame IoU $0.747$. Thus this section establishes both the
