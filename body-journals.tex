...
\begin{taggedeqn}{P$'$}{Pprime}
    ...
    \mathcal{L}_{\mathrm{GP}}(\mathbf{z};W,H,\mathbf{y}_{\mathrm{obs}})
    \le
...
\]
are used only for expert selection and candidate seeding. Their
purpose is to reduce expert-selection sensitivity to the real--simulation gap in
...
terms rather than by a separate expert-selection formula. Selecting an expert from observed
...
therefore reduce real--simulation-gap sensitivity during expert selection
...
\end{aligned}
\end{equation}
The implementation also records $I_{\mathrm{rms}}(\mathbf{z})=
...
$I_i=I_{\mathrm{rms}}(\mathbf{z}_i)$, $M_i$ be the largest per-frame
absolute whitened residual, and $q_i$ the support diagnostic. The point
...
expert validation counts have min/median/max $0/8/430$, with the $40$
...
Expert BAE diagonal variance
  & included in $\Sigma_{\mathrm{forward}}$
  & BAE rows for $2{,}775/2{,}775$ experts; expert validation count
...
Tab.~\ref{tab:rheo-recovery} lists the two-setup replay outputs
from the local GP surrogate trained on the quality-controlled
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{Real-material two-setup replay results from the GP
retained-set inverse. $E_{\mathrm{flow}}$ is the log-stress RMS
...
Material  & $(W_1,H_1)$ & $(W_2,H_2)$
          & $\hat n$ & $\hat\eta$ & $\hat\sigma_Y$ & $E_{\mathrm{flow}}$ & Time \\
\midrule
...
\begin{figure*}[!htbp]
    ...
    surrogate on the six rheometer-reference materials, following the layout
...
from six material groups. One Oats held-out pair contains flow-front
predictions but no binary-mask replay outputs, so it is included in the
flow-front rows and excluded from the mask rows. The largest mask
...
and the XOR difference. The Oats pair without binary-mask replay
outputs is excluded from this visual panel but remains in the
flow-front metric rows of
